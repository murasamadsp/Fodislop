...
namespace Fodinae.UI.Builders
{
    public class GridPacketBuilder : PacketUIBuilderBase
    {
        ...
        private VisualElement BuildGrid(GridPacket gridPkt, PacketUIBuilder builder)
        {
            ...
            {
                var row = 0; var col = 0; var rowSpan = 1; var colSpan = 1;
                if (childPacket.AttachedProperties != null)
                {
                    foreach (var prop in childPacket.AttachedProperties)
                    {
                        if (prop.Key == "Grid.Row" && int.TryParse(prop.Value, out var r)) row = r;
                        if (prop.Key == "Grid.Column" && int.TryParse(prop.Value, out var c)) col = c;
                        if (prop.Key == "Grid.RowSpan" && int.TryParse(prop.Value, out var rs)) rowSpan = rs;
                        if (prop.Key == "Grid.ColumnSpan" && int.TryParse(prop.Value, out var cs)) colSpan = cs;
                    }
                }
            ...
            onGeometryChanged = (evt) =>
            {
                ...
                for (var c = 1; c < columnStarts.Length; c++) columnStarts[c] = columnStarts[c - 1] + columnTracks[c - 1];
                for (var r = 1; r < rowStarts.Length; r++) rowStarts[r] = rowStarts[r - 1] + rowTracks[r - 1];
